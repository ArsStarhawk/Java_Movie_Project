...
public class AddFilmListener implements ActionListener
{
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        String insertStatement = String.format("INSERT INTO film "
                ...
                , titleStr, descriptionStr, yearEntered, languageStr, origLanguageStr, rentalDurationStr,
                                               rentalRateFinal, movieLengthStr, replacementCostStr, ratingStr,
                                               specialFeatureStr);
        ...
        try
        {
            ...
            for (String actorName : view.selectedActors)
            {
                ...
                String selectActorId = String
                        .format("SELECT actor_id FROM actor WHERE first_name = \"%s\" and last_name = \"%s\";",
                                firstName, lastName);
                myRslt = myStmt.executeQuery(selectActorId);
                myRslt.next();
                int actorId = myRslt.getInt(1);
                String insertActor = String
                        .format("INSERT INTO film_actor (actor_id, film_id) VALUES (%d, %d);", actorId, updatedId);
                int ret = myStmt.executeUpdate(insertActor);
            }

            //Insert film_categories
            String insertStatement2 = String
                    .format("INSERT INTO film_category (film_id, category_id) VALUES (%d, %d);", updatedId,
                            categoryStr);
